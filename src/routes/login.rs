...
pub async fn login(
    ...
    if let Some(user) = user {
        ...
        if Argon2::default().verify_password(input.password.as_bytes(), &parsed_hash).is_ok() {
            ...
            let access_token = try_or_http_err!(generate_access_token(user.id, user.email.as_str(), state.jwt_secret.as_str()));
            let refresh_token = try_or_http_err!(generate_refresh_token(user.id, user.email.as_str(), state.jwt_secret.as_str()));
